...
class TokenSale:
    """Manages token sales using a bonding curve pricing model"""
    ...
    async def buy_tokens(
        ...
        """Purchase tokens at the current bonding curve price"""
        if amount <= 0:
...
class TokenSaleProxy:
    ...
    async def buy_tokens(
        ...
        print(f"Buying {amount} tokens for {buyer}")
        return await self._implementation.buy_tokens(amount, buyer, commitment)
...
async def setup_ico(cluster: str = os.environ.get('CLUSTER', ClusterType.DEVNET)):
...
async def run_example_transactions(token_sale_proxy: TokenSaleProxy):
    ...
    try:
        await token_sale_proxy.buy_tokens(100, buyer)
